use crate::poseidon::{commit_ext_reference_impl, commit_reference_impl, compress_reference_impl};
...
pub(crate) fn verify_merkle_proof_packed<C: PseudoCompressionFunction<Digest, 2> + Sync, Digest>(
    c: &C,
    comm: Digest,
    mut index: usize,
    leaf: Digest,
...
#[derive(Debug)]
pub struct PoseidonCommitmentDataPacked<F: Field, Ext: ExtensionField<F>, const OUT: usize> {
        ...
        + Sync,
{
        ...
        Transcript: Writer<F> + Writer<[F; OUT]>,
    {
        ...
        #[cfg(debug_assertions)]
        {
            ...
            crate::poseidon::verify_merkle_proof(
                &self.compress,
                data.commitment(),
                index,
                leaf,
        ...
        Transcript: Reader<F> + Reader<[F; OUT]>,
    {
        ...
        verify_merkle_proof_packed::<C, [F; OUT]>(&self.compress, comm, index, leaf, &witness)?;
        Ok(row
            .iter()
...
> MerkleTreeExt<F, Ext> for PackedPoseidonMerkleTree<F, H, C, OUT>
{
    ...
    ) -> Result<PoseidonCommitmentDataPacked<F, Ext, OUT>, Error>
    where
