use crate::sumcheck::prover::ProverStack;
use crate::sumcheck::verifier::{SumcheckVerifier, VerifierStack};
...
fn test_zkvm() {
    ...
    layout.write_virtual_eval(&mut transcript).unwrap();
    let mut prover = layout.new_prover(&mut transcript).unwrap();
    ...
    let mut stack = VerifierStack::<F, Ext>::new(k_svo);
    ...
    let _ = layout.read_virtual_eval(&mut transcript).unwrap();
    let (mut verifier, _) = SumcheckVerifier::<F, Ext>::new(&mut transcript, layout).unwrap();
...
fn test_batch_sumcheck3() {
    ...
    for _ in 0..1 {
        ...
        let mut prover = stack.layout().new_prover(&mut transcript).unwrap();
        ...
        let (mut verifier, _) = SumcheckVerifier::new(&mut transcript, stack.layout()).unwrap();
        ds.iter()
            .zip(n_eqs.iter())
            .zip(n_pows.iter())
            .for_each(|((&d, &n_eq), &n_pow)| {
