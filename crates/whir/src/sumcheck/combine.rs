...
pub(crate) mod eq {
    ...
    use p3_util::log2_strict_usize;
    use rayon::prelude::*;
    use {common::utils::TwoAdicSlice, poly::Point};
...
pub(crate) mod pow {
    ...
    #[tracing::instrument(skip_all, fields(vars = vars.len(), k = out.k()))]
    pub(crate) fn combine_pows_scaled<F: Field, Ext: ExtensionField<F>>(
        out: &mut [Ext],
        vars: &[F],
        scales: &[Ext],
    ) {
...
mod test {
    ...
    use crate::sumcheck::combine::{combine_claims, combine_claims_packed};
    use crate::sumcheck::test::{make_eq_claims, make_pow_claims};
    use crate::sumcheck::{EqClaim, PowClaim};
    use common::field::*;
    use common::utils::{VecOps, unpack};
    use p3_util::log2_strict_usize;
    use poly::{Point, Poly};
    use rand::RngExt;
    use transcript::test_transcript::TestWriter;
