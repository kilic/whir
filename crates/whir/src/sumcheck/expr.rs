...
pub(crate) fn coeffs<A, B>(poly: &[A], weights: &[B]) -> (B, B)
where
...
#[derive(Debug, Clone)]
pub struct Expression<F: Field, Ext: ExtensionField<F>> {
    poly: Poly<Ext>,
...
impl<F: Field, Ext: ExtensionField<F>> Expression<F, Ext> {
    pub fn new(poly: Poly<Ext>, weights: Poly<Ext>) -> Self {
        Self {
            poly,
            weights,
    ...
    {
        let (c0, c2) = coeffs(&self.poly, &self.weights);
